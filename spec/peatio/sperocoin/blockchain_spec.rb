RSpec.describe Peatio::Sperocoin::Blockchain do
  context :features do
    it 'defaults' do
      blockchain1 = Peatio::Sperocoin::Blockchain.new
      expect(blockchain1.features).to eq Peatio::Sperocoin::Blockchain::DEFAULT_FEATURES
    end

    it 'override defaults' do
      blockchain2 = Peatio::Sperocoin::Blockchain.new(cash_addr_format: true)
      expect(blockchain2.features[:cash_addr_format]).to be_truthy
    end

    it 'custom feautures' do
      blockchain3 = Peatio::Sperocoin::Blockchain.new(custom_feature: :custom)
      expect(blockchain3.features.keys).to contain_exactly(*Peatio::Sperocoin::Blockchain::SUPPORTED_FEATURES)
    end
  end

  context :configure do
    let(:blockchain) { Peatio::Sperocoin::Blockchain.new }
  ...
  context :latest_block_number do
    ...
    let(:blockchain) do
      Peatio::Sperocoin::Blockchain.new.tap {|b| b.configure(server: server)}
    end
  ...
  context :build_transaction do
    ...
    context 'three vout tx' do
      ...
      let(:blockchain) do
        Peatio::Sperocoin::Blockchain.new.tap { |b| b.configure(currencies: [currency]) }
      end
    ...
    context 'multiple currencies' do
      ...
      let(:blockchain) do
        Peatio::Sperocoin::Blockchain.new.tap do |b|
  ...
  context :fetch_block! do
    ...
    let(:server_without_authority) { 'http://127.0.0.1:19332' }
    let(:blockchain) do
      Peatio::Sperocoin::Blockchain.new.tap { |b| b.configure(server: server, currencies: [currency]) }
    end
  ...
  context :load_balance_of_address! do
    ...
    context 'address with balance is defined' do
      it 'requests rpc listaddressgroupings and finds address balance' do
        ...
        result = blockchain.load_balance_of_address!(address, :spero)
    ...
    context 'address is not defined' do
      it 'requests rpc listaddressgroupings and do not find address' do
        ...
        expect{ blockchain.load_balance_of_address!(address, :spero)}.to raise_error(Peatio::Blockchain::UnavailableAddressBalanceError)
      end
    end
    ...
    context 'client error is raised' do
      ...
      it 'raise wrapped client error' do
        expect{ blockchain.load_balance_of_address!('anything', :spero)}.to raise_error(Peatio::Blockchain::ClientError)
      end
    end
  end
end
